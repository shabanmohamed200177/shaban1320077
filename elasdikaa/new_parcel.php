...
<div class="box-section">
    <div class="box-title"><span class="box-icon"><i class="fa-solid fa-sack-dollar"></i></span> ملخص الدفع الحالي</div>
    <?php
        $cod_amount__v = (float)($parcel_data['cod_amount'] ?? 0);
        $shipping_fees__v = (float)($parcel_data['shipping_fees'] ?? 0);
        $shipping_payer__v = (string)($parcel_data['shipping_payer'] ?? 'sender');
        $paid_amount__v = (float)($parcel_data['paid_amount'] ?? 0);
        $total_to_collect__v = $cod_amount__v + ($shipping_payer__v === 'recipient' ? $shipping_fees__v : 0);
        $remaining__v = max(0.0, $total_to_collect__v - $paid_amount__v);
        $payment_status__v = $paid_amount__v >= $total_to_collect__v ? 'paid' : ($paid_amount__v > 0 ? 'partial_paid' : 'unpaid');
    ?>
    <div class="row g-3 text-center">
        <div class="col-md-4">
            <div class="p-3 rounded bg-light">
                <div class="text-muted small">الإجمالي المطلوب تحصيله</div>
                <div class="fs-5 fw-bold text-success" id="edit_summary_total"><?php echo number_format($total_to_collect__v, 2); ?> جنيه</div>
            </div>
        </div>
        <div class="col-md-4">
            <div class="p-3 rounded bg-light">
                <div class="text-muted small">المدفوع</div>
                <div class="fs-5 fw-bold text-primary" id="edit_summary_paid"><?php echo number_format($paid_amount__v, 2); ?> جنيه</div>
            </div>
        </div>
        <div class="col-md-4">
            <div class="p-3 rounded bg-light">
                <div class="text-muted small">المتبقي</div>
                <div class="fs-5 fw-bold <?php echo $remaining__v > 0 ? 'text-danger' : 'text-success'; ?>" id="edit_summary_remaining"><?php echo number_format($remaining__v, 2); ?> جنيه</div>
            </div>
        </div>
    </div>
    <div class="text-center mt-2">
        <span class="badge bg-<?php echo $payment_status__v === 'paid' ? 'success' : ($payment_status__v === 'partial_paid' ? 'warning' : 'secondary'); ?>">
            حالة الدفع: <?php echo $payment_status__v === 'paid' ? 'مدفوع بالكامل' : ($payment_status__v === 'partial_paid' ? 'مدفوع جزئياً' : 'غير مدفوع'); ?>
        </span>
    </div>
</div>
<?php endif; ?>
