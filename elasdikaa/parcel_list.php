    ...
    $(document).ready(function() {
        ...
        $(document).on('click', '.change_status_btn', function() {
            ...
            $('#payment_note_input').val('');
                        ...
                        $('#modal_shipping_fees').text('0.00');
                        $('#modal_shipping_payer').text('');
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.status === 'success') {
                        ...
                        const cod = parseFloat(data.cod_amount || 0);
                        const ship = parseFloat(data.shipping_fees || 0);
                        const payer = String(data.shipping_payer || 'sender');
                        const paid = parseFloat(data.paid_amount || 0);
                        const totalToCollect = cod + (payer === 'recipient' ? ship : 0);
                        const remaining = Math.max(0, totalToCollect - paid);
                        $('#modal_total_amount_change').text(totalToCollect.toFixed(2));
                        $('#modal_paid_amount_change').text(paid.toFixed(2));
                        $('#modal_remaining_amount_change').text(remaining.toFixed(2));
                        $('#modal_shipping_fees').text(ship.toFixed(2));
                        $('#modal_shipping_payer').text(payer === 'sender' ? 'المرسل' : 'المستلم');
        ...
        $(document).on('click', '.pay_parcel_options_btn', function() {
            const parcelId = $(this).data('id');
            const codAmount = parseFloat($(this).data('cod')) || 0;
            const paidAmount = parseFloat($(this).data('paid')) || 0;
            const shippingFees = parseFloat($(this).data('shipping-fees')) || 0;
            const shippingPayer = String($(this).data('shipping-payer') || 'sender');

            let totalToCollectForModal = codAmount;
            if (shippingPayer === 'recipient') {
                totalToCollectForModal += shippingFees;
            }
            let remainingAmount = Math.max(0, totalToCollectForModal - paidAmount);
